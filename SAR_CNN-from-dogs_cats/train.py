...
batch_size = 32
#Prepare input data
classes = ['2S1','BRDM_2','D7','SLICY','T62','ZIL131','ZSU_23_4']           #图片的分类标签
...
def create_fc_layer(input,          
    ...
    layer=tf.nn.dropout(layer,keep_prob=0.7)            #用dropout处理过拟合
    if use_relu:
        layer = tf.nn.relu(layer)
    return layer
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples/batch_size) == 0: 
            ...
            saver.save(session, './ZIL-ZSU-model/ZIL-ZSU.ckpt',global_step=i)
    total_iterations += num_iteration
