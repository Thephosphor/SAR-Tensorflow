...
def inference(images, batch_size, n_classes):
    ...
    返回:
    输出tensor与计算的logits，float，[batch_size, n_classes]
    '''
...
def trainning(loss, learning_rate):
    ...
    '''
    '''训练Op，这个函数返回的Op是必须传递给'sess.run()'调用进行模型训练。
    参数:
    loss:loss tensor，来自于losses()
    返回:
    train_op:用于训练的op
    '''
...
def evaluation(logits, labels):
  ...
  labels:labels tensor，int32 - [batch_size]，值在区间[0,NUM_CLASSES)内
  返回:
  一个正确预测int32 tensor的标量，包含示例数(超出batch_size)
  '''
